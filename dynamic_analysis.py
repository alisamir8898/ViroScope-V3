...
class DynamicAnalyzer:
    ...
    def _execute_sample(self, file_path):
        """Execute the sample and monitor its behavior in real-time"""
        ...
        # قائمة لتجميع اتصالات الشبكة الحية أثناء عمل السكربت
        self.live_network_activity = []
        
        start_time = time.time()
        
        try:
            # تجهيز الأوامر بناءً على الامتداد (كما هي في كودك)
            file_extension = os.path.splitext(file_path)[1].lower()
            if file_extension == '.exe' and self.is_windows:
                command = [file_path]
            elif file_extension == '.ps1' and self.is_windows:
                command = ["powershell.exe", "-ExecutionPolicy", "Bypass", "-File", file_path]
            else:
                command = [file_path] if self.is_windows else ["python", file_path]

            # 1. تشغيل العملية بدون Blocking
            ...
            # 2. بدء الـ Monitor Thread وتمرير الـ PID له فوراً
            self.monitor_queue.put(process.pid)
            monitor_thread = threading.Thread(target=self._process_monitor_thread)
            monitor_thread.daemon = True
            monitor_thread.start()
            
            # 3. حلقة الرصد النشط (Active Polling Loop)
            # ستستمر في العمل طالما أن العملية لم تنتهِ ولم تتجاوز الـ Timeout
            while process.poll() is None:
                if time.time() - start_time > self.timeout:
                    logger.warning(f"Process timed out, terminating.")
                    process.kill()
    ...
    def _analyze_behavior(self, execution_info, created_processes, file_changes, network_activity):
        ...
        # 1. تحليل إنشاء العمليات (تعديل مرن للرصد العام)
        if len(created_processes) > 0:
            indicators.append(f"Spawned {len(created_processes)} child processes during execution")
        
        if len(created_processes) > 3:
            indicators.append("Multiple process creation detected")
            
        # فحص أسماء العمليات المشبوهة
        suspicious_names = ['powershell', 'cmd', 'wscript', 'cscript', 'schtasks', 'regedit', 'taskkill', 'calc', 'notepad'] # أضفنا الحاسبة والمفكرة للاختبار فقط
        ...
        # فحص الـ Command line
        for proc in created_processes:
            cmdline = ' '.join(proc.get('cmdline', [])).lower()
            if any(x in cmdline for x in ['hidden', '-w hidden', '/hidden', '-windowstyle hidden']):
                indicators.append("Process launched with hidden window (potential stealth/trojan technique)")
            ...
            if 'downloadstring' in cmdline or 'downloadfile' in cmdline:
                indicators.append("PowerShell download command detected")
                
        # 2. تحليل حركة الملفات
        if len(file_changes['created']) > 10:
            indicators.append("Created multiple files in a short period")
            
        executable_extensions = ['.exe', '.dll', '.bat', '.ps1', '.vbs', '.js', '.locked']
        for file_path in file_changes['created']:
            ext = os.path.splitext(file_path)[1].lower()
            if ext in executable_extensions:
                indicators.append(f"Created executable file: {os.path.basename(file_path)}")
            if ext == '.locked':
                indicators.append("File extension changed to locked (potential ransomware activity)")
                
        # 3. تحليل حركة الشبكة
        if len(network_activity) > 0:
            indicators.append(f"Established {len(network_activity)} active network connections")
            
        suspicious_ports = [4444, 8080, 443, 80, 1337, 666]
        for conn in network_activity:
            remote = conn.get('remote_address', '')
            if remote:
                # التحقق من وجود بورت مشبوه
                if any(f":{port}" in str(remote) or str(remote).endswith(str(port)) for port in suspicious_ports):
                    indicators.append(f"Connected to suspicious network port: {remote}")
                
        # 4. تتبع الـ Timeout
        if execution_info.get('terminated', False):
            indicators.append("Process exceeded execution timeout")
            
        # 5. تحليل مخرجات النص (طريقة ممتازة لربط الميزات بـ determine_malware_type)
        combined_output = (execution_info.get('stdout', '') + execution_info.get('stderr', '')).lower()
        if 'encrypt' in combined_output or 'ransom' in combined_output:
            indicators.append("Application output contains ransomware indicators (encrypt/ransom)")
        if 'launching visible dynamic behaviors' in combined_output:
            indicators.append("Triggered custom sandbox behavior testing pattern")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python dynamic_analysis.py <file_path>")
        exit(1)
